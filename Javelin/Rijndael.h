...
#define _RIJNDAEL_NO_ERROR_                 0
#define _RIJNDAEL_ERROR_NOT_INITIALISED_    1
#define _RIJNDAEL_ERROR_EMPTY_KEY_          2
#define _RIJNDAEL_ERROR_WRONG_KEY_LENGTH_   3
#define _RIJNDAEL_ERROR_WRONG_BLOCK_LENGTH_ 4

class CRijndaelException
{
private:
    int m_nError;
public:
    int GetError() const { return m_nError; }
    CRijndaelException( int nError ){ m_nError = nError; };
};
//unsigned char* align( unsigned char* pData, int nLen, int* pnNewLen );
...
public:
	...
	//BYTE* align( BYTE* pData, int nLen, int* pnNewLen, HANDLE hFile ) const;
private:
	//Auxiliary Function
	void Xor(char* buff, char const* chain)
	{
		if(false==m_bKeyInit)
			throw CRijndaelException(_RIJNDAEL_ERROR_NOT_INITIALISED_);//exception(sm_szErrorMsg1);
...
public:
	...
	{
		if(false==m_bKeyInit)
			throw CRijndaelException(_RIJNDAEL_ERROR_NOT_INITIALISED_);
		return m_keylength;
	}
